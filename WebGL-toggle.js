(function(global) {
    ...
    function waitForEntry(callback, maxAttempts = 100) {
        let attempts = 0;
        const checkEntry = () => {
            if (global.Entry && typeof global.Entry === 'object') {
                console.log("Entry object detected. Initializing WebGL toggler...");
                callback();
            } else {
                attempts++;
                if (attempts < maxAttempts) {
                    setTimeout(checkEntry, 100); // 100ms마다 체크
                } else {
                    console.error("Entry object not found after maximum attempts. WebGL toggler will not initialize.");
                }
            }
        };
        checkEntry();
    ...
    function initializeToggler() {
        if (global.Entry._webglTogglerInitialized) {
            console.log("WebGL toggler already initialized.");
            return;
        }
        global.Entry._webglTogglerInitialized = true;
    ...
    } // initializeToggler 함수 끝

    // Entry가 이미 있으면 즉시 실행, 없으면 대기
    if (global.Entry && typeof global.Entry === 'object') {
        console.log("Entry object already exists. Initializing WebGL toggler immediately...");
        initializeToggler();
    } else {
        console.log("Entry object not found yet. Waiting for Entry to load...");
        waitForEntry(initializeToggler);
    }
})(window);